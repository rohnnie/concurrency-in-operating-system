...
//array of mutexes, with size defined as NUM_BUCKETS

/*we're making a mutex per bucket, because the threads require access on a bucket level,
not on a table level*/
...
double now() {
  ...
  return tv.tv_sec + tv.tv_usec / 1000000.0;
    ...
    //we're only dividing usec by 1000000, and it's because we're converting it to seconds (10^-6)
}
...
void insert(int key, int val) {
  ...
  table[i] = e;
  //storing e as a whole into table[3], assuming i==3
